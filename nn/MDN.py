...
class MDN(nn.Module):
    ...
    def learn(self, inputs, cond_inputs, weights=None):
        ...
        # divide train & val
        x, y, w = inputs, cond_inputs, torch.zeros(len(inputs)).to(inputs.device)+1.0 if weights is None else weights
        n = len(x)
        n_val = int(0.85*n)
        idx = torch.randperm(n)
        x_train, y_train= x[idx[0:n_val]], y[idx[0:n_val]]
        x_val, y_val = x[idx[n_val:n]], y[idx[n_val:n]]
        w_train, w_val = w[idx[0:n_val]], w[idx[n_val:n]]
